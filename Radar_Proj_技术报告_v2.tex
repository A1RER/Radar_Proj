...
\begin{equation}
  ...
  \label{eq:echo}
\end{equation}
其中，第一个指数项为LFM脉冲压缩相位，第二个指数项包含了距离与多普勒信息，$n(\hat{t}, t_m)$为加性高斯白噪声。
...
\rowcolor{rowalt}
ISAR系统类 & \texttt{ISARImagingSystem.m} & --- \\
...
\begin{enumerate}
  \item \textbf{阶段一（近期）}：引入真实5G信道采集数据，完成实测回波预处理与成像验证；在不同SNR条件下系统评估算法性能；
  \item \textbf{阶段二（中期）}：扩充数据集规模至5000+样本，引入旋转、平移、噪声注入等数据增强策略，系统评估各分类器的泛化能力与计算效率；
  \item \textbf{阶段三（中长期）}：实现多基站协同定位系统联调，结合实际基站部署场景评估TDOA/AOA融合精度，优化GDOP；
  \item \textbf{长期目标}：凝练成果，投稿信号处理或雷达领域相关期刊/会议论文。
\end{enumerate}
...
下一步将重点向实测数据验证与多基站系统联调两个方向推进，为5G通感一体化低空安防应用提供技术储备。
